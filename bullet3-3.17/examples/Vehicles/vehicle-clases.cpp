...
float max_EngineForce = 1000.f;  //this should be engine/velocity dependent
float max_BreakingForce = 100.f;
...
btRaycastVehicle* Hinge2Vehicle::createVagon(btRaycastVehicle* parent_vehicle)
{
	...
	// void Hinge2Vehicle::renderScene(), similar a como ya se hace con el coche
...
void Hinge2Vehicle::initPhysics()
{
	...
	//btScalar camDist = m_guiHelper->getRenderInterface()->getActiveCamera()->getCameraDistance();
	//m_guiHelper->getRenderInterface()->getActiveCamera()->setCameraDistance(16.0f);

	btVector3 groundExtents(250, 5, 250);  //(50, 50, 50);
	...
	/// create vehicle
	{
		...
		btVector3 connectionPointCS0(CUBE_HALF_EXTENTS - (0.3 * wheel_Width), connectionHeight, 2 * CUBE_HALF_EXTENTS - wheel_Radius);

		m_vehicle->addWheel(connectionPointCS0, wheelDirection_CS0, wheelAxle_CS, suspension_RestLength, wheel_Radius, m_tuning, isFrontWheel);
		connectionPointCS0 = btVector3(-CUBE_HALF_EXTENTS + (0.3 * wheel_Width), connectionHeight, 2 * CUBE_HALF_EXTENTS - wheel_Radius);
